...
$Body = "Данные для согласования за: `n"+$($FilesToMail | % {$_.Substring(5,6)} | Sort)
...
if ($Debug) {$FilesToMail >>$DF}
if ($Debug) {$Line >>$DF}
Mail -To $To -Subject "Согласование генерации" -Body $Body -AttachmentLiteralPath $FilesToMail
$To = ?? {$a2} {"asqe@rduvolgograd.ru"}
$Body = $Body+"`nНаправлены субъекту $SubLbl на согласование..."
if ($Debug) {$To >>$DF; $Body >>$DF; $Line >>$DF}
Mail -To $To -Subject "Согласование генерации" -Body $Body
if ($Cnt -ne 0)
{
